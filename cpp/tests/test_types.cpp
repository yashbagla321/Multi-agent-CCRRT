...
TEST(Types, MakePredictionFromWaypoints_ConstantVariance) {
    const auto prediction = makePredictionFromWaypoints({{0, 0}, {1, 0}, {2, 0}}, 0.2);
    ASSERT_EQ(prediction.nodes.size(), 3u);
    EXPECT_DOUBLE_EQ(prediction.nodes[0].variance, 0.2);
    EXPECT_DOUBLE_EQ(prediction.nodes[1].variance, 0.2);
    EXPECT_DOUBLE_EQ(prediction.nodes[2].variance, 0.2);
}
